...
const { splitShared } = await load("split");
const { renderClosed } = await load("render");
...
const render = (event, orders, paypayUrl) =>
  renderClosed(event, splitShared(event.shared_costs, orders), paypayUrl);
...
test("各行に 弁当代 + 割前 = 送る金額 が出る", () => {
  const lines = description(render(closed(), six())).split("\n");
...
test("送る金額が splitShared の結果と一致する", () => {
  ...
  const text = description(render(closed(), orders));
...
test("合計行に弁当代・共通費用・総額が出て、内訳のラベルが付く", () => {
  const text = description(render(closed(), six()));
...
test("共通費用が複数行なら全部のラベルが出る", () => {
  ...
  const text = description(render(closed(costs), six()));
...
test("メンションは一切入らない", () => {
  const body = render(closed(), six(), "https://paypay.me/xxxx");
...
test("paypay_url があれば送金先が出る", () => {
  const text = description(render(closed(), six(), "https://paypay.me/xxxx"));
  assert.ok(text.includes("送金先 → https://paypay.me/xxxx"));
});

test("paypay_url が無ければ送金先の行ごと出ない", () => {
  for (const url of [null, undefined]) {
    assert.ok(!description(render(closed(), six(), url)).includes("送金先"));
  }
});

test("components は [支払った] [未払いに戻す ▼] の2つだけ", () => {
  const ids = customIds(render(closed(), six()));
  assert.deepEqual(ids, ["paid", "unpaid_select"]);
  // 注文フェーズのボタンは1つも残らない
